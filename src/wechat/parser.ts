...
export class WechatParser {
    ...
    private async crawlPage(index: number) {
        ...
        try {
            res = await crawlerService.search({
    ...
    private async parsePageItem(item: PageItem, next: (article: WechatArticle) => void) {
        ...
        while (infos.length > 0) {
            ...
            if (dbArticle) {
                console.log(`正在更新文章数据:${JSON.stringify({ id: info.id, title: info.title,  url: info.url })}`);
            } else {
                console.log(`正在解析文章数据:${JSON.stringify({ id: info.id, title: info.title,  url: info.url })}`);
            }
